...
const PlanEdit = () => {
  ...
  useEffect(() => {
    planService.getById(id).then(res => {
      ...
      if (p) setForm({
        ...
        maxUsers: p.maxUsers, maxWorkflows: p.maxWorkflows, maxProjects: p.maxProjects ?? 3,
        isActive: p.isActive, isPopular: p.isPopular, color: p.color || '#2563eb',
        order: p.order || 0, hasAI: p.hasAI ?? false, hasAnalytics: p.hasAnalytics ?? false,
        hasAdvancedForms: p.hasAdvancedForms ?? false, hasAPIAccess: p.hasAPIAccess ?? false,
        hasSSO: p.hasSSO ?? false, hasSMSNotif: p.hasSMSNotif ?? false,
      });
    }).catch(console.error).finally(() => setPageLoad(false));
  }, [id]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      await planService.update(id, { ...form, price: parseFloat(form.price), maxUsers: parseInt(form.maxUsers), maxWorkflows: parseInt(form.maxWorkflows), maxProjects: parseInt(form.maxProjects), order: parseInt(form.order), features: form.features.filter(f => f.trim()) });
  ...
  if (pageLoad) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%', gap:'10px', color:'#94a3b8', fontFamily:'Inter, -apple-system, sans-serif' }}>
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden', fontFamily:'Inter, -apple-system, sans-serif', background:'#f0f4f8' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 24px', borderBottom:'1px solid #e2e8f0', background:'#fff', flexShrink:0, boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
          <Link to="/dashboard/superadmin/plans" style={{ display:'flex', alignItems:'center', gap:'5px', background:'#f1f5f9', border:'none', padding:'7px 12px', borderRadius:'8px', cursor:'pointer', fontWeight:600, color:'#64748b', fontSize:'12.5px', textDecoration:'none' }}
          ...
          <div>
            <h1 style={{ margin:0, fontSize:'15px', fontWeight:800, color:'#0f172a', display:'flex', alignItems:'center', gap:'7px' }}>
              <i className="ri-edit-line" style={{ color:'#2563eb' }}></i>Modifier le plan
              {form.name && <span style={{ background: form.color || '#2563eb', color:'#fff', fontSize:'11px', fontWeight:700, padding:'2px 10px', borderRadius:'999px', marginLeft:'4px' }}>{form.name}</span>}
            </h1>
            <p style={{ margin:0, fontSize:'11px', color:'#94a3b8' }}>Mettez à jour les informations du plan</p>
          </div>
        </div>
        <div style={{ display:'flex', gap:'8px' }}>
          ...
          <button type="submit" form="plan-edit-form" disabled={loading} style={{ display:'inline-flex', alignItems:'center', gap:'7px', padding:'8px 20px', borderRadius:'8px', border:'none', background: loading ? '#94a3b8':'#2563eb', color:'#fff', fontSize:'13px', fontWeight:700, cursor: loading ? 'not-allowed':'pointer', boxShadow:'0 2px 8px rgba(37,99,235,.25)' }}>
            <i className={loading ? 'ri-loader-4-line':'ri-save-line'} style={{ fontSize:'15px' }}></i>
      ...
      <form id="plan-edit-form" onSubmit={handleSubmit} style={{ flex:1, overflow:'auto', padding:'14px 24px' }}>
            ...
            <Section icon="ri-information-line" title="Informations de base" color="#2563eb">
              <div style={{ display:'grid', gridTemplateColumns:'1fr 80px', gap:'10px' }}>
                <Field label="Nom du plan" icon="ri-text" required>
                  <input name="name" value={form.name} onChange={handleChange} required disabled={loading} style={inp} onFocus={focus} onBlur={blur} />
                </Field>
            ...
            <Section icon="ri-money-dollar-circle-line" title="Tarification" color="#059669">
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                <Field label="Prix (dt)" icon="ri-price-tag-3-line" required>
                  <input type="number" name="price" value={form.price} onChange={handleChange} min="0" step="0.01" required disabled={loading} style={inp} onFocus={focus} onBlur={blur} />
                </Field>

...
const IconArchive  = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="5" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"/><line x1="10" y1="12" x2="14" y2="12"/></svg>;
                          ...
                          <div style={{display:'flex',gap:'7px',alignItems:'center',flexShrink:0,flexWrap:'wrap'}}>
                            ...
                            {['draft','completed','rejected'].includes(wf.status) && (
                              <button onClick={()=>handleDelete(wf)} disabled={deleting===wf._id}
                                title="Archiver"
                                style={{width:'36px',height:'36px',display:'flex',alignItems:'center',justifyContent:'center',background:'#FFFBEB',color:'#D97706',border:'1.5px solid #FDE68A',borderRadius:'9px',fontWeight:700,cursor:deleting===wf._id?'not-allowed':'pointer',opacity:deleting===wf._id?0.5:1}}>
                                {deleting===wf._id ? <IconLoader/> : <IconArchive/>}
                              </button>
                            )}
                          </div>
                        </div>
